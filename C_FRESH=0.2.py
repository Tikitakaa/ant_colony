...
class HybridAntColony:
    ...
    def __init__(self, z):
        ...
        self.N_MAX = 5
        self.C_FRESH = 0.2              #新引入参数
        self.gamma = 5                  #朱申俊引入的betaf
    ...
    def init(self):
        ...
        for i in range(0, len(self.customers)-1):
            for j in range(i+1, len(self.customers)):
                ...
                # 按照朱申俊方法设计启发式因子
                self.herustic[i][j] = self.herustic[j][i] = self.customers[j].q_i #(1 / self.graph[i][j]) * self.customers[j].q_i * (1 / (self.customers[j].t_load * 60))     # * 60
    ...
    def a_star(self):
        ...
        while len(self.untreated) != 0:
            ...
            for i in self.untreated:
                ...
                c_41 = self.P * (self.customers[i].q_i * (1 - math.exp(-self.TRANS_REDUCT_RATIO / (self.gamma * self.C_FRESH) * (self.customers[i].t_i - 6.5))))
                c_42 = self.P * route.load * (1 - math.exp(-self.LOAD_REDUCT_RATIO * self.customers[i].t_load) / (self.gamma * self.C_FRESH))
                c_4 = c_41 + c_42
                c_5 = 0.0
...
if __name__ == "__main__":
    ...
    for i in hybrid_ant_colony.best_solution.routes:
        print(i.customers)
    print(hybrid_ant_colony.best_solution.totalcost)
    print(hybrid_ant_colony.best_solution.c1)
    print(hybrid_ant_colony.best_solution.c2)
    print(hybrid_ant_colony.best_solution.c3)
    print(hybrid_ant_colony.best_solution.c4)
    print(hybrid_ant_colony.best_solution.c5)
    print(str(convergence_times))
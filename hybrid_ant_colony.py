...
class HybridAntColony:
    ...
    def __init__(self, z):
        ...
        self.alpha = 1                  #信息素因子
        self.beta = 3                   #启发素因子
        self.sita = 0.9                 #信息素挥发因子
        self.q_0 = 0.6                  #参数q0
        self.Q = 100                    #信息素总量
        self.ant = 10                   #蚂蚁数量
        self.lambda_phe = 1.2           #lambda取1.8
    ...
    def a_star(self):
        ...
        while len(self.untreated) != 0:
            ...
            for i in self.untreated:
                ...
                c_41 = self.P * (self.customers[i].q_i * (1 - math.exp(-self.TRANS_REDUCT_RATIO * (self.customers[i].t_i - 8))))
        ...
        for i in self.best_solution.routes:
            for j in range(0,len(i.customers)-1):
                ...
                #有改动
                self.best_solution.c4 +=self.P * ((self.customers[i.customers[j+1]].q_i * (1 - math.exp(-(self.TRANS_REDUCT_RATIO / (self.gamma * self.C_FRESH)) * (self.customers[i.customers[j+1]].t_i - 8)))) + (i.load - self.customers[i.customers[j+1]].q_i) * (1 - math.exp(-(self.LOAD_REDUCT_RATIO / (self.gamma * self.C_FRESH)) * self.customers[i.customers[j+1]].t_load)))

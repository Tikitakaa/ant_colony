    ...
    def a_star(self):
        ...
        for i in self.best_solution.routes:
            for j in range(0,len(i.customers)-1):
                ...
                #有改动
                self.best_solution.c4 +=self.P * ((self.customers[i.customers[j+1]].q_i * (1 - math.exp(-(self.TRANS_REDUCT_RATIO) * (self.customers[i.customers[j+1]].t_i - 6.5)))) + (i.load - self.customers[i.customers[j+1]].q_i) * (1 - math.exp(-(self.LOAD_REDUCT_RATIO) * self.customers[i.customers[j+1]].t_load)))
    ...
    def construct_solution(self):
        ...
        for i in self.solution.routes:
            for j in range(0,len(i.customers)-1):
                ...
                #有改动
                self.solution.c4 += self.P * ((self.customers[i.customers[j+1]].q_i * (1 - math.exp(-(self.TRANS_REDUCT_RATIO) * (self.customers[i.customers[j+1]].t_i - 6.5)))) + (i.load - self.customers[i.customers[j+1]].q_i) * (1 - math.exp(-(self.LOAD_REDUCT_RATIO) * self.customers[i.customers[j+1]].t_load)))
                if (self.customers[i.customers[j+1]].e_i - self.customers[i.customers[j+1]].t_i) > 0:
...
if __name__ == "__main__":
    ...
    with open('C:\\Users\\98600\\Desktop\\hybrid_ant_colony.txt', 'a', encoding='utf-8') as fp:
        for i in hybrid_ant_colony.best_solution.routes:
